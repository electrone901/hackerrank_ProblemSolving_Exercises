...
output: 1487800378 # since the increase in the number of people
	...
	PSEUDO CODE:
	1: Make sure you read and understand the problem, the details are important 
	   to solve the problem
	2: declare var:
		2.1: count, maxCount, maxTime
		2.2: loop through the ar:
			- if(ar[i][2]==1) count +=  ar[i][2]
			- else count -=  ar[i][2]
    3: if is the end of the period then calculate:maxCount, maxTime
    	3.1: if(i<ar.length && data[i][0] !== data[i+1][0])
    		  - check if(count > maxVisitors) {
				    maxVisitors = count;
				    maxTime = data[i][0]; 
				}
*/
...
function findBusiestPeriod(data) {
  ...
  for(let i=0; i < data.length; i++) {
    ...
    // isSameTime then skip to the next i
    if(i < data.length-1 && data[i][0] == data[i+1][0]) {
     continue;
    }
    // if doesn't skip will check maxCount
    if(count > maxCount) {
      maxCount = count;
      maxTime = data[i][0];
    }
