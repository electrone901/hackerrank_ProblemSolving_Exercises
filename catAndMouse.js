...
For example, cat  is at position  and cat  is at . If mouse  is at position , it is  units from cat  and  
unit from cat . Cat  will catch the mouse.
...
z: an integer, Mouse 's position
	...
	PSEDUDO CODE:
	1 - if(absolute value z-x == absolute value z-y) return 'Mouse C'
	2 - else if(absolute value z-x < absolute value z-y) return 'Mouse A'
	3 - else return 'Mouse B'
*/

function catAndMouse(x,y,z) {
	if(Math.abs(z-x) == Math.abs(z-y)) return 'Mouse C';
	else if(Math.abs(z-x) < Math.abs(z-y)) return 'Cat A';
	else return 'Cat B';
}
catAndMouse(1,2,3);
